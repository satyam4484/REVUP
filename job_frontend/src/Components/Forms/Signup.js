...
import {validateEmail,createUser,getOtp} from "../../network/agent";
...
const Signup = () => {
  ...
  const createAccountHandler = (e) => {
    ...
    createUser({
      ...
      if (response.error === false) {
        toggleSpin();
        dispatch({ type: "CLEAR" });
        setMessage(true, "success", response.message, "Verify Your email");
        setTimeout(() => {
          // navigate("/auth/login");
  ...
  return (
    ...
    {modal && <OtpForm show={modal} onHide={() =>setModal(false)} reset={false} />}
